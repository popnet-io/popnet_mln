...
Finally, most graph libraries are not designed to handle large tabular node attributes alongside multilayer edge data. These are common in population-scale social networks where the registers also allow including attributes of people (e.g., information on age, gender, education, or income).
...
1. Efficiently store sparse multilayer edges encoded into a single sparse adjacency matrix.
...
General-purpose graph libraries such as NetworkX [@networkx] and igraph [@igraph] are widely used and provide rich algorithmic toolkits. However, multilayer handling is not the first priority of these packages, therefore, related operations are not implemented or not efficient due to lack of explicit multilayer support.
